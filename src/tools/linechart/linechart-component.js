...
const LCComponent = Component.extend({
  ...
  init(config, context) {
    ...
    this.model_binds = {
      ...
      "change:time.playing": function() {
        ...
        if (_this.model.time.playing && utils.isTouchDevice() && !_this.tooltip.classed("vzb-hidden")) _this.tooltip.classed(
					"vzb-hidden", true);
      },
  ...
  readyOnce() {
    ...
    this.yInfoEl.on("mouseover", function() {
      ...
      const coord = utils.makeAbsoluteContext(this, this.farthestViewportElement)(rect.x - 10, rect.y + rect.height +
				10);
  ...
  updateSize() {
    ...
    if (this.height <= 0 || this.width <= 0) return utils.warn(
			"Line chart updateSize() abort: vizabi container is too little or has display:none");
  ...
  shadeID: "shade"
          ...
          const xy = _this.prev_steps.map((frame, i) => [frame, _this.all_values[frame] ? _this.all_values[frame].axis_y[
            d[KEY]] : null])
						.filter(d => d[1] || d[1] === 0);
      ...
      _this.verticalNow
        ...
        .duration(_this.duration)
        .ease(d3.easeLinear)
        .attr("transform", "translate(" + _this.xScale(d3.min([_this.model.marker.axis_x.zoomedMax, _this.time])) +
					",0)");
